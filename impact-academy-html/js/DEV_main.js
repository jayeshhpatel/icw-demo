...
(function ($) {
  $(document).ready(function() {
    ...
    if ($('.sticky-header').length) {
      ...
      /* Slider - Card View | with-tooltip */
      if ($('.with-tooltip').length) {
          $(".card-slider").on("click", ".with-tooltip", function(e) {
          //$(".card-slider .slick-active").delegate("click",".with-tooltip",function(e){
          e.preventDefault();
      ...
      if ($('.register-form').length) {
          $(document).on('keyup blur change', '.register-form #step1 input, .register-form #step1 select', function () {
              if ($('#rf_country').val() != '_none' && $('#rf_fname').val() && $('#rf_surname').val() && $('#rf_email').val()) {
          ...
          $(document).on('keyup blur change', '.register-form #step2 input, .register-form #step2 select', function () {
              if ($("#rf_acceptcheck").is(':checked') && $("#rf_speciality").val() != '_none' && $("#rf_speciality").val() != '_none') {
                  $('.register-form #step2 .form-action .btn').prop('disabled', false);
              } else {
                  $('.register-form #step2 .form-action .btn').prop('disabled', 'disabled');
              }
          });

          $(document).on("click", "._next-step1", function() {
              if ($('#rf_country').val() && $('#rf_fname').val() && $('#rf_surname').val() && $('#rf_email').val()) {
